...
%type <str> AlgorithmClause
%type <str> algorithm_parameter_list
%type <str> algorithm_parameter
%type <str> algorithm_parameters
%type <str> forecast_clause
%type <str> opt_mergeclause
%type <str> opt_forecast_clause
%type <str> opt_on_clause
%type <str> opt_algorithm_clause
%type <str> opt_storage_clause
%type <str> DisAggStmt
%type <str> where_list
%type <str> CreateModelGraphStmt
%type <str> graphAttributeList
%type <str> graphAttribute
%type <str> correlationClause
%type <str> correlationList
%type <str> correlation
%type <str> ReestimateModelGraphModelsStmt
%type <str> DropModelGraphStmt
%type <str> CreateDisAggSchemeStmt
%type <str> opt_a_expr
%type <str> opt_key_clause
%type <str> FillModelGraphStmt
%type <str> fill_method_clause
...
/* orig_tokens */
 ...
 ARRAY AS ASC ASOF ASSERTION ASSIGNMENT ASYMMETRIC AT ATTRIBUTES AUTHORIZATION AVERAGE
 ...
 CONTENT_P CONTINUE_P CONVERSION_P COPY CORRELATION COST CREATE CREATEDB
 ...
 DICTIONARY DIMENSION DISABLE_P DISAGGREGATE DISAGGSCHEME DISCARD DISTINCT DO DOCUMENT_P DOMAIN_P
 DOUBLE_P DROP
 ...
 NOT NOTHING NOTIFY NOTNULL NOWAIT NULL_P NULLIF NULLS_P NUMBER NUMERIC

 OBJECT_P OF OFF OFFSET OIDS OLD ON ONLY OPERATOR OPTION OPTIONS OR
 ORDER OUT_P OUTER_P OVER OVERLAPS OVERLAY OWNED OWNER

 PARAMETERS PARSER PARTIAL PARTITION PASSWORD PLACING PLANS POSITION
 ...
 SAVEPOINT SCHEMA SCROLL SEARCH SEASON SECOND_P SECURITY SELECT SEQUENCE
 SERIALIZABLE SERVER SESSION SESSION_USER SET SETOF SHARE
 SHOW SIMILAR SIMPLE SMALLINT SOME STABLE STANDALONE_P START STATEMENT
 STATISTICS STRATEGY STDIN STDOUT STORAGE STORE STRICT_P STRIP_P SUBSTRING SUPERUSER_P
 ...
 %nonassoc IDENT
%nonassoc CSTRING
%nonassoc UIDENT PARTITION RANGE ROWS
 %left Op OPERATOR
 %nonassoc NOTNULL
 %nonassoc ISNULL
...
;
 ...
 IndexStmt:
 CREATE index_opt_unique INDEX index_name ON qualified_name access_method_clause '(' index_params ')' opt_reloptions OptTableSpace where_clause
 { 
 $$ = cat_str(13,make_str("create"),$2,make_str("index"),$4,make_str("on"),$6,$7,make_str("("),$9,make_str(")"),$11,$12,$13);
}
|  CREATE index_opt_unique INDEX CONCURRENTLY index_name ON qualified_name access_method_clause '(' index_params ')' opt_reloptions OptTableSpace where_clause
 { 
...
;
 ...
 PRINT MODELGRAPH
 { 
 $$ = make_str("print modelgraph");
}
;


 StoreStmt:
 STORE MODELGRAPH AS ColId WITHOUT FILLING
 { 
 $$ = cat_str(3,make_str("store modelgraph as"),$4,make_str("without filling"));
}
|  STORE MODELGRAPH AS ColId WITH FILLING AS ColId
 { 
 $$ = cat_str(4,make_str("store modelgraph as"),$4,make_str("with filling as"),$8);
}
|  STORE FILLING AS ColId
 { 
 $$ = cat_str(2,make_str("store filling as"),$4);
}
;


 RestoreStmt:
 RESTORE MODELGRAPH ColId WITHOUT FILLING
 { 
 $$ = cat_str(3,make_str("restore modelgraph"),$3,make_str("without filling"));
}
|  RESTORE MODELGRAPH ColId WITH FILLING ColId
 { 
 $$ = cat_str(4,make_str("restore modelgraph"),$3,make_str("with filling"),$6);
}
|  RESTORE FILLING ColId
 { 
 $$ = cat_str(2,make_str("restore filling"),$3);
}
;
...
|  select_clause opt_sort_clause forecast_clause
 { 
 $$ = cat_str(3,$1,$2,$3);
}
...
;
 ...
 { 
 $$ = cat_str(8,make_str("create model"),$3,make_str("for forecast of"),$7,make_str("on"),$9,$10,$11);
}
;
 ...
 { 
 $$ = cat_str(4,make_str("algorithm"),$2,make_str("training_data"),$4);
}
|  ALGORITHM ColId PARAMETERS algorithm_parameter_list TRAINING_DATA select_with_parens
 { 
 $$ = cat_str(6,make_str("algorithm"),$2,make_str("parameters"),$4,make_str("training_data"),$6);
}
;
...
|  algorithm_parameters ',' algorithm_parameter
 { 
 $$ = cat_str(3,$1,make_str(","),$3);
}
;
 ...
 { 
 $$ = cat_str(4,make_str("decompose window"),$3,make_str("season"),$5);
}
|  opt_forecast_clause opt_on_clause NUMBER Iconst opt_algorithm_clause opt_storage_clause opt_mergeclause
 { 
 $$ = cat_str(7,$1,$2,make_str("number"),$4,$5,$6,$7);
}
|  opt_forecast_clause opt_on_clause ASOF ecpg_sconst opt_algorithm_clause opt_storage_clause opt_mergeclause
 { 
 $$ = cat_str(7,$1,$2,make_str("asof"),$4,$5,$6,$7);
}
;
 ...
 CHOOSE FIRST_P
 { 
 $$ = make_str("choose first");
}
|  CHOOSE LAST_P
 { 
 $$ = make_str("choose last");
}
|  CHOOSE AVERAGE
 { 
 $$ = make_str("choose average");
}
| 
 { 
 $$=EMPTY; }
...
|  ALGORITHM ColId
 { 
 $$ = cat_str(2,make_str("algorithm"),$2);
}
|  ALGORITHM ColId PARAMETERS algorithm_parameter_list
 { 
 $$ = cat_str(4,make_str("algorithm"),$2,make_str("parameters"),$4);
}
;
...
;
 ...
 STORAGE OFF
 { 
 $$ = make_str("storage off");
}
|  STORAGE STATEMENT
 { 
 $$ = make_str("storage statement");
}
|  STORAGE TABLE
 { 
 $$ = make_str("storage table");
}
|  STORAGE MODELGRAPH
 { 
 $$ = make_str("storage modelgraph");
}
|  STORAGE MODELINDEX
 { 
 $$ = make_str("storage modelindex");
}
|  FORCE STORAGE STATEMENT
 { 
 $$ = make_str("force storage statement");
}
|  FORCE STORAGE TABLE
 { 
 $$ = make_str("force storage table");
}
|  FORCE STORAGE MODELGRAPH
 { 
 $$ = make_str("force storage modelgraph");
}
|  FORCE STORAGE MODELINDEX
 { 
 $$ = make_str("force storage modelindex");
}
| 
 { 
 $$=EMPTY; }
;
 ...
 { 
 $$ = cat_str(9,make_str("disaggregate"),$2,make_str("from"),$4,make_str("attributes ("),$7,make_str(") keys ("),$11,make_str(") strategy mult"));
}
;
...
;
 ...
 { 
 $$ = cat_str(5,make_str("create modelgraph"),$3,make_str("from"),$5,$6);
}
;
...
|  graphAttributeList ',' graphAttribute
 { 
 $$ = cat_str(3,$1,make_str(","),$3);
}
;
...
|  c_expr EXCLUDING '(' var_list ')'
 { 
 $$ = cat_str(4,$1,make_str("excluding ("),$4,make_str(")"));
}
;
 ...
 { 
 $$ = cat_str(2,make_str("correlation"),$2);
}
| 
 { 
 $$=EMPTY; }
...
|  correlationList ',' '(' correlation ')' correlation:
 graphAttribute
 { 
 $$ = $1;
}
|  correlation '>' graphAttribute
 { 
 $$ = cat_str(3,$1,make_str(">"),$3);
}
;
...
;
 ...
 { 
 $$ = cat_str(9,make_str("create disaggscheme for forecast of"),$6,make_str("on"),$8,$9,make_str("from"),$11,$12,$13);
}
;
...
;
 ...
 opt_key_clause:
 WITH KEY NumericOnly
 { 
 $$ = cat_str(2,make_str("with key"),$3);
}
| 
 { 
 $$=EMPTY; }
;
 ...
 { 
 $$ = cat_str(6,make_str("fill modelgraph for forecast of"),$6,make_str("on"),$8,$9,$10);
}
;
 ...
 METHOD TOPDOWN
 { 
 $$ = make_str("method topdown");
}
|  METHOD BOTTOMUP
 { 
 $$ = make_str("method bottomup");
}
|  METHOD GREEDY
 { 
 $$ = make_str("method greedy");
}
;
...
|  func_name '(' ALL expr_list ')' over_clause
 { 
 $$ = cat_str(5,$1,make_str("( all"),$4,make_str(")"),$6);
}
|  func_name '(' DISTINCT expr_list ')' over_clause
 { 
 $$ = cat_str(5,$1,make_str("( distinct"),$4,make_str(")"),$6);
}
|  func_name '(' '*' ')' over_clause
 { 
...
|  DISAGGREGATE
 { 
 $$ = make_str("disaggregate");
}
|  DISAGGSCHEME
 { 
 $$ = make_str("disaggscheme");
}
|  DISTINCT
 { 
 $$ = make_str("distinct");
