...
transformCreateModelStmt(CreateModelStmt *stmt,const char *queryString)
{
	 ...
	 */
	output = findTargetlistEntry(NULL,stmt->outputcolumn,&(query->targetList),ORDER_CLAUSE);
	...
	timeColumns = list_make1(findTargetlistEntry(NULL,(Node *) lfirst(elem),&(query->targetList),ORDER_CLAUSE));

	for_each_cell(elem,elem->next) {
		lappend(timeColumns,findTargetlistEntry(NULL,(Node *) lfirst(elem),&(query->targetList),ORDER_CLAUSE));
	}
