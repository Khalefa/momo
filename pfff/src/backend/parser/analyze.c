...
static Query *transformExplainStmt(ParseState *pstate,
					 ExplainStmt *stmt);
static Query *transformPrintStmt(ParseState *pstate,
					 PrintStmt *stmt);
...
transformStmt(ParseState *pstate, Node *parseTree)
{
	...
	switch (nodeTag(parseTree))
	{
		...
		case T_PrintStmt:
			result = transformPrintStmt(pstate,
										  (PrintStmt *) parseTree);
			break;
...
transformSelectStmt(ParseState *pstate, SelectStmt *stmt)
{
	...
	{
		if (IsA(stmt->forecastExpr, ForecastExpr)) {
			
			qry->forecastExpr = (Node*) transformForecastClause(pstate, (ForecastExpr*) stmt->forecastExpr, qry);

			//parse trainingdata by hand, if these are explicitly given
			if(((ForecastExpr*)(stmt->forecastExpr))->algorithm->trainingdata!=NULL)
			{
				querytree_list = pg_analyze_and_rewrite(((ForecastExpr*)(stmt->forecastExpr))->algorithm->trainingdata,((ForecastExpr*)(stmt->forecastExpr))->sourcetext,0,0);
				//store the now analyzed and rewritten query for later processing back to the trainingdata-node
				((ForecastExpr*)(qry->forecastExpr))->algorithm->trainingdata = lfirst(list_head(querytree_list));
			}
			//MMIndexTimeSeries(qry);
		}
		else if (IsA(stmt->forecastExpr, DecomposeExpr))
			qry->forecastExpr = (Node*) transformDecomposeClause(pstate, (DecomposeExpr*) stmt->forecastExpr, qry);
	}
	//qry->forecastExpr = NULL;
	
	if (stmt->disAggExpr)
	{
		qry->disAggExpr = (Node*) transformDisAggClause(pstate, (DisAggExpr *) stmt->disAggExpr, qry);
	}
 ...
 * Dummy for printing stuff
 */

static Query *
transformPrintStmt(ParseState *pstate, PrintStmt *stmt)
{
	Query	   *result;
	result = makeNode(Query);
	result->commandType = CMD_UTILITY;
