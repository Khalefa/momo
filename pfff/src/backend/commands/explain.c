...
static void show_cm(Plan *cmplan,
			   const char *qlabel,
			   StringInfo str, int indent, ExplainState *es);
			   
static void show_sf(Plan *cmplan,
			   const char *qlabel,
			   StringInfo str, int indent, ExplainState *es);

static void show_CreateModelGraphStmt(Plan *cmplan,
			   const char *qlabel,
			   StringInfo str, int indent, ExplainState *es);


void
PrintOutput(PrintStmt *stmt, const char *queryString,
			 ParamListInfo params, DestReceiver* dest)
			 {
	
				 TupOutputState *tstate;
				 tstate = begin_tup_output_tupdesc(dest, ExplainResultDesc(NULL));
				do_text_output_multiline(tstate,printModelGraph());
				...
				int indent, ExplainState *es)
{
	...
	switch (nodeTag(plan))
	{
		...
		{
			pname = "Create ForecastModel";
			break;
		}
		case T_Decompose:
			pname = "Decompose Additiv";
			break;
		case T_CreateModelGraphStmt:
		{
			pname = "Create Modelgraph";
			break;
		}
		default:
			pname = "???";
	...
	switch (nodeTag(plan))
	{
		case T_SingleForecast:
		{
			show_sf(plan,"", str, indent, es);
			break;
			
		}
		case T_CreateForecastModel:
		{
			show_cm(plan,"Output", str, indent, es);
			break;
			
		}
		case T_CreateModelGraphStmt:
		{
			show_CreateModelGraphStmt(plan, "", str, indent, es);
			break;
		}
		case T_IndexScan:
...
show_sf(Plan *sfplan,
			   ...
			   StringInfo str, int indent, ExplainState *es)
{
	SingleForecast *sf=((SingleForecast*)sfplan);
	ModelInfo* mi;
	ListCell *lc1;
	int buildM=-1;
	int store;
	...
	foreach(lc1,sf->candidatemodelInfos)
	{
		mi=lfirst(lc1);
		if(buildM==-1)
		{
			buildM=mi->buildModel;
			store=mi->storeModel;
		}
		
		appendStringInfo(str, "  parameters%i: [",cnt);
		appendStringInfo(str,"model name=%s;",mi->modelName);
		appendStringInfo(str, "  model type=%s; ",getModelTypeAsString(mi->forecastMethod));
		if((mi->storeModel == 4|| mi->storeModel == 14) && mi->mix && (nodeTag(mi->mix)==T_IndexNode || nodeTag(mi->mix)==T_AggNode))
			appendStringInfo(str,"where expr=%s;",getWhereExprAsString((ModelGraphIndexNode *)mi->mix));//FAKEFLAG
//			appendStringInfo(str,"where expr=product='Phones' AND state='Ohio';");
		parameterListToString(str,mi->parameterList);
			appendStringInfo(str,"buildModel=%i;",mi->buildModel);
		appendStringInfo(str,"implicit=false;");
		
		appendStringInfo(str,"disag key=%f;", mi->disaggkey); //FAKEFLAG
		appendStringInfo(str, " ],");
		cnt++;
	}
	appendStringInfo(str, "  Input: ");
	switch(store)
			{
				case 0:appendStringInfo(str, "ModelIndex,");break;
				case 10:appendStringInfo(str, "Force Modelindex,");break;
				case 1:appendStringInfo(str, "OFF,");break;
				case 11:appendStringInfo(str, "OFF,");break;
				case 2:appendStringInfo(str, "Hash,");break;
				case 12:appendStringInfo(str, "Force Hash,");break;
				case 3:appendStringInfo(str, "Systemtable,");break;
				case 13:appendStringInfo(str, "Force Systemtable,");break;
				case 4:appendStringInfo(str, "ModelGraph,");break; //FAKEFLAG
				case 14:appendStringInfo(str, "Force ModelGraph,");break;
				default:break;
			}
		appendStringInfo(str, "  buildModel: %i,",buildM);
		appendStringInfo(str, "  modelCount: %i,",sf->candidatemodelInfos->length);
		appendStringInfo(str, "\n");
			   ...
			   StringInfo str, int indent, ExplainState *es)
{
	...
	storeM=((CreateForecastModel*)cmplan)->modelInfo->storeModel;
			switch(storeM)
			{
				case 0:appendStringInfo(str, "ModelIndex,");break;
				case 10:appendStringInfo(str, "Force Modelindex,");break;
				...
				case 3:appendStringInfo(str, "SystemTable,");break;
				case 13:appendStringInfo(str, "Force SystemTable,");break;
				case 4:appendStringInfo(str, "ModelGraph,");break;
				case 14:appendStringInfo(str, "Force ModelGraph,");break;
				default:break;
			}
	appendStringInfo(str, "  model type=%s, ",getModelTypeAsString(((CreateForecastModel*)cmplan)->modelInfo->forecastMethod));
	appendStringInfo(str, "  Parameters: [");
	parameterListToString(str,((CreateForecastModel*)cmplan)->modelInfo->parameterList);
	appendStringInfo(str,"model name=%s;",((CreateForecastModel*)cmplan)->modelInfo->modelName);
	appendStringInfo(str, "  model type=%s; ",getModelTypeAsString(((CreateForecastModel*)cmplan)->modelInfo->forecastMethod));
	appendStringInfo(str,"buildModel=%i;",((CreateForecastModel*)cmplan)->modelInfo->buildModel);
	if(((CreateForecastModel*)cmplan)->end==-2)
	appendStringInfo(str,"implicit=true;");
	else
		appendStringInfo(str,"implicit=false;");
	appendStringInfo(str,"disag key=%f;", ((CreateForecastModel*)cmplan)->modelInfo->disaggkey); //FAKEFLAG
	appendStringInfo(str, " ],");
	appendStringInfo(str, "\n");
}

/*
 * Show create model graph stmt
 */
static void
show_CreateModelGraphStmt(Plan *mgsplan,
			   const char *qlabel,
			   StringInfo str, int indent, ExplainState *es)
{
	int			i;
	...
	CreateModelGraphStmt *mgs=((CreateModelGraphStmt*)mgsplan);
	for (i = 0; i < indent; i++)
		appendStringInfo(str, "  ");
	appendStringInfo(str, "  Attributes: [");
	foreach(lc, mgs->graphAttributeList){
		appendStringInfo(str,"%s;", ((TargetEntry *)((GraphAttribute *)lfirst(lc))->attributeCol)->resname);
	}
	appendStringInfo(str, " ]");
	appendStringInfo(str, "\n");
}
