...
static node_pos get_btree_node(btree * btree,int key);
...
int btree_delete_key(btree * btree,bt_node * subtree,int key) {
...
*/
node_pos  get_btree_node(btree * btree,int key) {
	node_pos kp;
...
*/
bt_key_val * btree_search(btree * btree,int key) {
...
*	@return The max key 
*/
int btree_get_max_key(btree * btree) {
	node_pos node_pos;
...
*	@return The max key 
*/
int btree_get_min_key(btree * btree) {
	node_pos node_pos;
...
static void print_single_node(btree *btree, bt_node * node) {
	...
	while(i < node->nr_active) {
		printf("%i(%d) ", btree->value(node->key_vals[i]->key),
			node->level);
		i++;
	}
