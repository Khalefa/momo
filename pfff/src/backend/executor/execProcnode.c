...
ExecInitNode(Plan *node, EState *estate, int eflags)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_SingleForecast:
			result = (PlanState *) ExecInitSingleForecast((SingleForecast *) node,
															 estate, eflags);
			break;
		case T_CreateForecastModel:
			result = (PlanState *) ExecInitCreateForecastModel((CreateForecastModel *) node, estate,eflags);
			break;
		case T_Decompose:
			result = (PlanState *) ExecInitDecompose((Decompose *) node,
															 estate, eflags);
			break;
		case T_DisAgg:
			result = (PlanState *) ExecInitDisAgg((DisAgg *) node, estate, eflags);
			break;
...
ExecProcNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_LimitState:
			...
			break;
		case T_SingleForecastState:
			result = ExecSingleForecast((SingleForecastState *) node);
			break;
		case T_CreateForecastModelState:
			result = ExecCreateForecastModel((CreateForecastModelState *) node);
			break;
		case T_DecomposeState:
			result = ExecDecompose((DecomposeState *) node);
			break;
		case T_DisAggState:
			result = ExecDisAgg((DisAggState *) node);
			break;
		default:
			elog(ERROR, "unrecognized node type: %d", (int) nodeTag(node));
			result = NULL;
