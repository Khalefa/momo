...
_equalQuery(Query *a, Query *b)
{
	...
	COMPARE_NODE_FIELD(forecastExpr);
...
static bool
_equalForecastExpr(ForecastExpr *a, ForecastExpr *b)
{
	COMPARE_STRING_FIELD(sourcetext);
	COMPARE_NODE_FIELD(time);
	COMPARE_NODE_FIELD(algorithm);
	COMPARE_NODE_FIELD(timeCols);
	COMPARE_NODE_FIELD(measureCols);
	COMPARE_NODE_FIELD(categoryCols);
	COMPARE_NODE_FIELD(whereExpr);
	COMPARE_SCALAR_FIELD(granularity);
	COMPARE_SCALAR_FIELD(aggType);
...
static bool
_equalDecomposeExpr(DecomposeExpr *a, DecomposeExpr *b)
{
	COMPARE_NODE_FIELD(model);
	COMPARE_NODE_FIELD(targetlist);
...
static bool
_equalModelNode(ModelNode *a, ModelNode *b)
{
...
static bool
_equalConnection(Connection *a, Connection *b)
{
	COMPARE_SCALAR_FIELD(id);
...
equal(void *a, void *b)
{
	...
	switch (nodeTag(a))
	{
		...
		case T_JoinExpr:
			...
			break;
		case T_ForecastExpr:
			retval = _equalForecastExpr(a, b);
			break;
		case T_DecomposeExpr:
			retval = _equalDecomposeExpr(a, b);
			break;
		...
		case T_ModelNode:
			retval = _equalModelNode(a, b);
			break;
		case T_Connection:
			retval = _equalConnection(a, b);
			break;

		default:
			elog(ERROR, "unrecognized node type: %d (equalfuncs)",
				 (int) nodeTag(a));
