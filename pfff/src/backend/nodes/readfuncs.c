...
_readAlgorithmParameter(void)
{
	READ_LOCALS(AlgorithmParameter);
...
{
	READ_LOCALS(A_Const);
	((A_Const*)(local_node))->val.type=T_Integer;
	token = pg_strtok(&length);		/* skip :fldname */ \
	token = pg_strtok(&length);		/* get field value */ \
	local_node->val.val.ival = atoi(token);
	token = pg_strtok(&length);		/* skip :fldname */ \
	token = pg_strtok(&length);		/* get field value */ \
...
{
	READ_LOCALS(A_Const);
	((A_Const*)(local_node))->val.type=T_String;
	token = pg_strtok(&length);		/* skip :fldname */ \
	token = pg_strtok(&length);		/* get field value */ \
	local_node->val.val.str = nullable_string(token,length);
	token = pg_strtok(&length);		/* skip :fldname */ \
	token = pg_strtok(&length);		/* get field value */ \
	local_node->location = atoi(token);
	READ_DONE();
...
_readDecomposeExpr(void)
{
	READ_LOCALS(DecomposeExpr);

	READ_NODE_FIELD(model);
	READ_NODE_FIELD(targetlist);
...
#define MATCH(tokname, namelen) \
	...
	else if (MATCH("FORECASTEXPR", 12))
		return_value = _readForecastExpr();
	else if (MATCH("DECOMPOSEEXPR", 13))
