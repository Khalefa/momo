...
static void
_outSingleForecast(StringInfo str, SingleForecast *node)
{
	WRITE_NODE_TYPE("FORECAST");
...
static void
_outCreateForecastModel(StringInfo str,CreateForecastModel *node)
{
	WRITE_NODE_TYPE("CREATEFORECASTMODEL");
...
static void
_outDecompose(StringInfo str, Decompose *node)
{
	WRITE_NODE_TYPE("DECOMPOSE");
...
static void
_outDecomposeExpr(StringInfo str, DecomposeExpr *node)
{
	WRITE_NODE_TYPE("DECOMPOSEEXPR");

	WRITE_NODE_FIELD(model);
	WRITE_NODE_FIELD(targetlist);
}


static void
_outModelInfo(StringInfo str, ModelInfo *node)
{
	WRITE_NODE_TYPE("MODELINFO");

	WRITE_NODE_FIELD(measure);
	WRITE_NODE_FIELD(time);
	WRITE_INT_FIELD(forecastMethod);
	WRITE_INT_FIELD(modelOid);
	WRITE_INT_FIELD(numForecastValues);
	WRITE_NODE_FIELD(model);
	WRITE_INT_FIELD(storeModel);
}
...
static void
_outAlgorithmParameter(StringInfo str, AlgorithmParameter *node){
	WRITE_NODE_TYPE("ALGORITHMPARAMETER");
...
static void
_outValue(StringInfo str, Value *value,bool flag) //flag: string wo düddel
{
	switch (value->type)
	{
		case T_Integer:
...
{
	if(node->val.type==T_Integer)
	{
		WRITE_NODE_TYPE("A_CONSTI");
	}
	else
	{
		WRITE_NODE_TYPE("A_CONSTS");
	}
	
	appendStringInfo(str, " :val ");
	_outValue(str, &(node->val),false);
	WRITE_LOCATION_FIELD(location);
}
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_FromExpr:
				...
				break;
			case T_ModelInfo:
				_outModelInfo(str, obj);
				break;
			case T_ForecastExpr:
				_outForecastExpr(str, obj);
				break;
			case T_DecomposeExpr:
				_outDecomposeExpr(str, obj);
				break;
			...
			default:
				 ...
				 */
				elog(WARNING, "could not dump unrecognized node type: %d (outfuncs)",
					 (int) nodeTag(obj));
				break;
		}
		appendStringInfoChar(str, '}');
