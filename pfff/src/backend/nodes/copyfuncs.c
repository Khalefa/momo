...
*/
static Decompose *
_copyDecompose(Decompose *from)
{
	Decompose *newnode = makeNode(Decompose);

	COPY_NODE_FIELD(model);
...
*/
static ModelInfo *
_copyModelInfo(ModelInfo *from){
	ModelInfo *newnode = makeNode(ModelInfo);

	COPY_SCALAR_FIELD(forecastMethod);
	COPY_STRING_FIELD(modelName);
	COPY_NODE_FIELD(time);
	COPY_NODE_FIELD(measure);
	COPY_SCALAR_FIELD(aggType);
	COPY_SCALAR_FIELD(granularity);
	COPY_SCALAR_FIELD(timestamp);
	COPY_SCALAR_FIELD(disaggkey);
	newnode->model = from->model;
	COPY_SCALAR_FIELD(numForecastValues);
	COPY_SCALAR_FIELD(buildModel);
	COPY_SCALAR_FIELD(storeModel);
	COPY_NODE_FIELD(mix);
	COPY_SCALAR_FIELD(modelOid);
	COPY_NODE_FIELD(parameterList);
	COPY_STRING_FIELD(trainingData);
	COPY_SCALAR_FIELD(errorSMAPE);
	COPY_SCALAR_FIELD(errorSSE);
	COPY_SCALAR_FIELD(errorML);
	COPY_SCALAR_FIELD(disAggKeyDenominator);
	newnode->algInfo = initAlgorithmInfo(from->algInfo->algName);
	newnode->algInfo->algName = from->algInfo->algName ? MemoryContextStrdup(CurrentMemoryContext, (from->algInfo->algName)) : (char *) ((void *)0);
 ...
 */
static ForecastExpr *
_copyForecastExpr(ForecastExpr *from)
{
 	ForecastExpr *newnode = makeNode(ForecastExpr);
 
 	COPY_STRING_FIELD(sourcetext);
 	COPY_NODE_FIELD(time);
 	COPY_NODE_FIELD(measure);
 	COPY_NODE_FIELD(algorithm);
	COPY_STRING_FIELD(modelName);
 	COPY_NODE_FIELD(timeCols);
 	COPY_NODE_FIELD(measureCols);
 	COPY_NODE_FIELD(categoryCols);
 	COPY_NODE_FIELD(whereExpr);
 	COPY_SCALAR_FIELD(granularity);
 	COPY_SCALAR_FIELD(aggType);
 ...
 * _copyDecomposeExpr
 */
static DecomposeExpr *
_copyDecomposeExpr(DecomposeExpr *from)
{
	DecomposeExpr *newnode = makeNode(DecomposeExpr);
...
static CreateDisAggSchemeStmt * _copyCreateDisaggScheme(CreateDisAggSchemeStmt* from)
{
	CreateDisAggSchemeStmt *newnode = makeNode(CreateDisAggSchemeStmt);
	COPY_NODE_FIELD(target);
	COPY_NODE_FIELD(source);
	COPY_NODE_FIELD(time);
	COPY_NODE_FIELD(measure);
	COPY_NODE_FIELD(algorithm);
	COPY_NODE_FIELD(disAggKey);
	COPY_NODE_FIELD(rTable);
	return newnode;
...
_copyAlgorithmClause(AlgorithmClause *from)
{
	AlgorithmClause			*newnode = makeNode(AlgorithmClause);

	COPY_STRING_FIELD(algorithmname);
	COPY_NODE_FIELD(algorithmparameter);
	return newnode;
}


//FIXME......
static CreateModelGraphStmt *_copyCreateModelGraph (CreateModelGraphStmt *from)
{
//	CreateModelGraphStmt *newnode=makeNode(CreateModelGraphStmt);  
//	COPY_STRING_FIELD(sourcetext);
//	COPY_NODE_FIELD(subquery);
//	COPY_NODE_FIELD(graphAttributeList);
//	COPY_NODE_FIELD(corrLists);
//	newnode->tDesc=CreateTupleDescCopy(from->tDesc);
//	COPY_NODE_FIELD(tupleList);
	return from;
	
}

static FillModelGraphStmt *_copyFillModelGraph(FillModelGraphStmt *from){

	FillModelGraphStmt *newnode = makeNode(FillModelGraphStmt);
	COPY_STRING_FIELD(sourcetext);
	COPY_NODE_FIELD(time);
	COPY_NODE_FIELD(measure);
	newnode->algorithm = _copyAlgorithmClause(from->algorithm);
	newnode->fillMethode = from->fillMethode;
	return newnode;
}

static PrintStmt* _copyPrintStmt(PrintStmt *from)
{	
	return from;
}


static DropModelGraphStmt* _copyDropModelGraphStmt(DropModelGraphStmt *from)
{	
	return from;
}

static StoreStmt* _copyStoreStmt(StoreStmt *from)
{
	StoreStmt *newnode = makeNode(StoreStmt);
	COPY_STRING_FIELD(name);
	COPY_STRING_FIELD(fillingname);
	return from;
}
static RestoreStmt* _copyRestoreStmt(RestoreStmt *from)
{
	RestoreStmt *newnode = makeNode(RestoreStmt);
	COPY_STRING_FIELD(name);
	COPY_STRING_FIELD(fillingname);
	return from;
}
...
_copyAlgorithmParameter(AlgorithmParameter *from)
{
	AlgorithmParameter		*newnode = makeNode(AlgorithmParameter);
...
copyObject(void *from)
{
	...
	switch (nodeTag(from))
	{
			 ...
			 * PLAN NODES
			 */
		case T_CreateDisAggSchemeStmt:
			retval= _copyCreateDisaggScheme(from);
			break;
		case T_PlannedStmt:
		...
		case T_FromExpr:
			...
			break;
		case T_ForecastExpr:
 			retval = _copyForecastExpr(from);
 			break;
		case T_DecomposeExpr:
		 	retval = _copyDecomposeExpr(from);
		 	break;
		case T_ModelInfo:
			retval = _copyModelInfo(from);
			break;
		...
		case T_AlgorithmClause:
			retval = _copyAlgorithmClause(from);
			break;
		case T_AlgorithmParameter:
			retval = _copyAlgorithmParameter(from);
			break;
		case T_CreateModelGraphStmt:
			retval= _copyCreateModelGraph(from);
			break;
		case T_FillModelGraphStmt:
			retval= _copyFillModelGraph(from);
		case T_PrintStmt:
			retval= _copyPrintStmt(from);
			break;
		case T_DropModelGraphStmt:
			retval= _copyDropModelGraphStmt(from);
			break;
		case T_StoreStmt:
			retval= _copyStoreStmt(from);
			break;
		case T_RestoreStmt:
			retval= _copyRestoreStmt(from);
			break;
		

		default:
			elog(ERROR, "unrecognized node type: %d (copyfuncs)", (int) nodeTag(from));
