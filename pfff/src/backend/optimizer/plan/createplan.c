...
Decompose *
make_decompose(Plan *lefttree, Node *decomposeExpr)
{
	Decompose  		*node = (Decompose*) makeNode(Decompose);
	Plan	   		*plan = &node->plan;
	DecomposeExpr 	*decompose = (DecomposeExpr*) decomposeExpr;

	/* for now just copy costs */
	copy_plan_costsize(plan, lefttree);

	plan->targetlist = (List*) decompose->targetlist;
	plan->qual = NIL;
	plan->lefttree = lefttree;
	plan->righttree = NULL;
	
	node->model = decompose->model;
	node->window = decompose->window;
	node->season = decompose->season;
...
SingleForecast *
make_forecast(Plan *lefttree, Node *forecastExpr, Query **qry)
{

	List * paraMeterList=NIL;
	AlgorithmParameter* arP;
	A_Const *arConst;
	Value *arVal;
	AlgorithmParameter* maP;
	A_Const *maConst;
	Value *maVal;
	SingleForecast  *node = makeNode(SingleForecast);
	Plan	   		*plan = &node->plan;
	...
	copy_plan_costsize(plan, lefttree);
	plan->plan_rows = forecast->length;
	
	// init plan parameters
	plan->targetlist = lefttree->targetlist;
	plan->qual = NIL;
	plan->lefttree = lefttree;
	plan->righttree = NULL;
	
	// init forecast horizon
	node->start = 1;
...
//	elog(WARNING, "create plan here !!"); 
	...
	//check date string
	//calculate end from targetDate if we have to
	if(node->end == 0){ //to forecast 0 values is nonsense
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("forecast length must be greater 0")));
	}
	else if((node->end == -1) && (strlen(node->targetDateString) == 0)) //no targetDate given
	{
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("targetDate mustn't be empty")));
	}
...
//elog(WARNING,"!!!3");
		...
		modelInfo->measure = (TargetEntry *) list_nth((*qry)->targetList, linitial_int(forecast->measureCols) - 1);
		modelInfo->aggType = forecast->aggType;
		// set modelname
	...
	//	elog(WARNING,"!!!4   %d",modelInfo->forecastMethod);
		...
		//check first if the ModelGraph should be used and forward the model to the ModelIndex if the forecasts whereExpr is not compatible with the ModelhGraph
		if((forecast->storeModel == 4) && (modelInfo->forecastMethod != R)) // modelGraph
		{
			if(!isModelGraphExistent())
			{
				forecast->storeModel=1;
				
			}
			else
			{
			//check compatibility and contingent forward to ModelIndex
			if(!modelGraphCompatible(forecast->whereExpr, *qry)){
				elog(ERROR, "At least one predicate was not compatible with the ModelGraph!");
			}
			modelInfo->mix = forecast->whereExpr;
			
						//find existing model and add it to the candidateModels
			FindModelGraphModels(modelInfo,&node->candidatemodelInfos,&possiblePath, &createModels,(*qry)->rtable,&(forecast->whereExpr));
			if((createModels)!=NIL && forecast->length!=-2 && !allow_ModelGraph_create)
				elog(ERROR,"There are models missing, enable allow_ModelGraph_create to allow the Modelgraph to create these Models");
			//TODO: Model Creation
			if((createModels)!=NIL && createModels->length>1 && forecast->length==-2)
				elog(ERROR,"Given WhereExpr identifies more than one modelnode");

					
			//found an AggregationScheme
			node->choose=4;
		...
		if ((forecast->storeModel == 0) && (modelInfo->forecastMethod != R)) // model index
		{
			...
			node->candidatemodelInfos = FindMatchingNode(modelInfo, forecast->whereExpr, (*qry)->rtable, node->targetDateString);
		} 
		else if ((forecast->storeModel == 2) && (modelInfo->forecastMethod != R)) // hash table
		{
			//Hell i am no longer working :-(
			found = SearchQueryString(modelInfo, forecast->sourcetext);

			if (found == false)
				node->sourcetext = forecast->sourcetext;
		}
		else if ((forecast->storeModel == 3) && (modelInfo->forecastMethod != R)) // system table
		{
			//Get list of possible models
			node->candidatemodelInfos=FindModel(forecast->whereExpr, modelInfo,node->targetDateString);
			//No models found
			if (node->candidatemodelInfos==NIL)
			{
				modelInfo->mix = forecast->whereExpr;
				node->sourcetext = forecast->sourcetext;
			}
			else
			{
				...
				{
					ModelInfo* mI = lfirst(lc);
					initForecastModel(mI, CurrentMemoryContext);
					loadModelParameters(mI, mI->modelOid);
				}
			}
		}
		else // do not store or Force Storage
		{
			if(forecast->storeModel==14 && (modelInfo->forecastMethod != R))
			{
				//TODO: the case of Foreced Storage to the ModelGraph
				//maybe incompatibility should not be handled with forwarding here?!
			}
			else if(forecast->storeModel==10 && (modelInfo->forecastMethod != R))
			{
				//Get list of possible models
				node->candidatemodelInfos = FindMatchingNode(modelInfo, forecast->whereExpr, (*qry)->rtable,node->targetDateString);
				if(node->candidatemodelInfos)
					{
						modelInfo->storeModel = 10; //there is something to overwrite
						modelInfo=((ModelInfo*)(lfirst(list_head(node->candidatemodelInfos)))); //set model, which should be overwritten
						node->candidatemodelInfos=NIL;
					}
				else
					modelInfo->storeModel = 0; //nothing to overwrite, use standard create
			}
			else if(forecast->storeModel==13 && (modelInfo->forecastMethod != R))
			{
				//Get list of possible models
				node->candidatemodelInfos=FindModel(forecast->whereExpr, modelInfo,node->targetDateString);
				//No models found
				...
				else
				{
					...
					foreach(lc, node->candidatemodelInfos)
					{
						...
						initForecastModel(mI, CurrentMemoryContext);
						loadModelParameters(mI, mI->modelOid);
						modelInfo=mI; //take a model to overwrite
					}
					modelInfo->storeModel = 13;
					node->candidatemodelInfos=NIL;
				}
			}
		}
	//	elog(WARNING,"!!!6");
		...
		} else 
		{
			if(forecast->length==-2)
				ereport(ERROR, (errmsg("A model like this already exists!")));
		...
		//still needed?
		if(forecast->algorithm->algorithmparameter==NIL && (modelInfo->forecastMethod==GretlArima || modelInfo->forecastMethod==ArModel) && modelInfo->storeModel>=10)
		{
			arP = makeNode(AlgorithmParameter);
			arP->key="ar";
	...
	{
		// TODO: support multiple time series in one select clause
		ereport(ERROR, (errmsg("multiple time series not supported")));
	}
...
CreateForecastModel
*make_create_forecast_model(Plan *lefttree, ModelInfo *model,Node *forecastExpr, Query **qry)
{

	CreateForecastModel *node = makeNode(CreateForecastModel);
	ForecastExpr 		*exp  = (ForecastExpr *) forecastExpr;

	if(model == NULL) {
		//TODO: model initialization
		// no model initialized yet -> can happen if creation and use don't occur in the same Query
	}
...
Node *
make_disAgg(PlannerInfo *root, Plan *lefttree, Node *disAggExpr, Query **qry){

	DisAgg  		*node = makeNode(DisAgg);
	Plan	   		*plan = &node->plan;
	DisAggExpr 		*disAgg = (DisAggExpr *) disAggExpr;
	TargetEntry 	*tle;
	List			*tlist = NIL; // the new TargetList
	List			*sortlist = NIL, *pathkeys = NIL;
	int				i;
	RangeTblEntry 	*attRTE;
	OpExpr 			*opex;
	Var 			*v;
	Oid 			ltOpr, eqOpr, colType;
	SortGroupClause *firstSortCl, *secondSortCl;
	PathKey 		*pk;
	AttrNumber 		*sortColIdx;
	Oid 			*sortOperators;
	bool 			*nullsFirst;
	...
	// if we just have one disaggregation-attribute we don't want a new first column containing it(KEEP THIS CONSISTENT WITH THE nodeDisAgg!!!)
	if(disAgg->attributes->length > 1){
		// add the attributeColumn to the new TargetList that we copy from lefttree
		opex = (OpExpr *)lfirst(list_head(disAgg->attributes));
		v = (Var *)lfirst(list_head(opex->args));
		// find a RTE that corresponds to the Var of the attributes, and this one contains the colName we need
		attRTE = rt_fetch(v->varno, (*qry)->rtable);

		// create a new TargetEntry that represents the attributeColumn...
		tle = makeTargetEntry((Expr *)v, 1, ((Value *)list_nth(attRTE->eref->colnames, v->varattno-1))->val.str, false);
		// ...and put it to the first column of the result...
		tlist = lappend(tlist, tle);
		// ...and copy all of the lefttree's targetentries
		for(i=0; i<lefttree->targetlist->length; ++i){
			tlist = lappend(tlist, list_nth(lefttree->targetlist, i));
		}
	...
	// just forward all given infos
	node->targetCol = disAgg->targetCol;
	node->attributes = disAgg->attributes;
	node->keys = disAgg->keys;
	node->strategy = disAgg->strategy;
	node->sourcetext = disAgg->sourcetext;

	// if we just have one disaggregation-attribute, keep the ordering
	// otherwise add a SortNode, and  sort the result by the disAgg-attribute AND the first col of lefttree
	if(disAgg->attributes->length > 1){
		// make sortclause for disAgg-attribute
		get_sort_group_operators(v->vartype, true, true, false, &ltOpr, &eqOpr, NULL);

		firstSortCl = makeNode(SortGroupClause);
		firstSortCl->tleSortGroupRef = assignSortGroupRef(tle, tlist);
		firstSortCl->eqop = eqOpr;
		firstSortCl->sortop = ltOpr;
		firstSortCl->nulls_first = false;

		sortlist = lappend(sortlist, firstSortCl);

		// make sortclause for first col of lefttree
		colType = ((Var *)((TargetEntry *)lfirst(list_head(lefttree->targetlist)))->expr)->vartype;
		get_sort_group_operators(colType, true, true, false, &ltOpr, &eqOpr, NULL);

		secondSortCl = makeNode(SortGroupClause);
		secondSortCl->tleSortGroupRef = assignSortGroupRef(tle, tlist);
		secondSortCl->eqop = eqOpr;
		secondSortCl->sortop = ltOpr;
		secondSortCl->nulls_first = false;

		sortlist = lappend(sortlist, (void*)secondSortCl);

		// generrate the pathkey for the two orderingsteps
		pathkeys = make_pathkeys_for_sortclauses(root, sortlist, tlist, false);

		// this cols of the targetlist should be sorted
		sortColIdx = palloc0(2*sizeof(AttrNumber));
		sortColIdx[0] = 1;
		sortColIdx[1] = 2;
		// they should be sorted with these operators
		sortOperators = palloc0(2*sizeof(Oid));
		pk = (PathKey *)list_nth(pathkeys, 0);
		sortOperators[0] = get_opfamily_member(pk->pk_opfamily, v->vartype, v->vartype, pk->pk_strategy);
		// Oid from first fact-column
		pk = (PathKey *)list_nth(pathkeys, 1);
		sortOperators[1] = get_opfamily_member(pk->pk_opfamily, colType, colType, pk->pk_strategy);
		// NULLs won't be in leading positions (normally there won't be any NULLs)
		nullsFirst = palloc0(2*sizeof(bool));
		nullsFirst[0] = false;
		nullsFirst[1] = false;

		return (Node *)make_sort(root, (Plan *)node, 2, sortColIdx, sortOperators, nullsFirst, -1.0);
	}
	return (Node *)node;
}
