...
set_plan_refs(PlannerGlobal *glob, Plan *plan, int rtoffset)
{
	...
	switch (nodeTag(plan))
	{
		...
		{
			SingleForecast	   *splan = (SingleForecast *) plan;
			
			if (splan->plan.lefttree != NULL)
				set_upper_references(glob, plan, rtoffset);
			else
			{
				splan->plan.targetlist = fix_scan_list(glob, splan->plan.targetlist, rtoffset);
			}
			break;
		}
		case T_CreateForecastModel:
		{
			CreateForecastModel		*splan = (CreateForecastModel *)plan;
			if (splan->plan.lefttree != NULL)
				set_upper_references(glob, plan, rtoffset);
			else
			{
...
set_upper_references(PlannerGlobal *glob, Plan *plan, int rtoffset)
{
	...
	{
		//in case of DisAgg we skip "fixing" the first TLE, because this one is not filled by sub_plan
		if(plan->type == T_DisAgg && !skipped){
			skipped = true;
