...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	double		dNumGroups = 0;
	Node *forecast = parse->forecastExpr;
		Node *disAgg = parse->disAggExpr;
	...
	/*
	 * As a last step, add the FORECAST node
	 */
	
	if (forecast)
	{
		if (IsA(forecast, ForecastExpr)) {
			result_plan = (Plan*)make_forecast(result_plan, forecast, &parse);

			//plan the already analyzed and rewritten trainingdata if there are any
			if(((ForecastExpr*)forecast)->algorithm->trainingdata != NULL){
				Plan* result2 = subquery_planner(root->glob, (Query*)((ForecastExpr*)forecast)->algorithm->trainingdata, NULL, false, tuple_fraction, NULL);
				//put the now planned trainingdata to righttree of the createModel(which is on the left of the forecast)
				result_plan->lefttree->righttree=result2;
			}
		}
		else
			result_plan =(Plan *) make_decompose(result_plan, forecast);
	}

	/*
	 * As a last step, add the DISAGGREGATION node
	 */
	
	if(disAgg){
		result_plan = (Plan *) make_disAgg(root, result_plan, disAgg, &parse);
	}
